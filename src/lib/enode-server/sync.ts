...
import type { EnodeCharger, EnodeInverter, EnodeWebhookEvent } from "@/lib/enode-server/enode";
import {
...
function chargerRow(
  ...
  return {
    ...
    charge_rate_kw: rate,
    battery_level_pct:
      ch.chargeState?.batteryLevel != null
...
export async function upsertDevice(row: DeviceRow): Promise<string | null> {
  ...
  const { error: ingestError } = await db.rpc("enode_ingest_telemetry", {
...
export async function enqueueWebhookEvent(
  ...
  const { error } = await db.from("enode_webhook_queue").insert({
...
export async function handleWebhookEvent(
  ...
  const { error: insertError } = await db.from("enode_events").insert({
    company_id: companyId,
    delivery_id: deliveryId,
    event_type: event.event,
    enode_user_id: enodeUserId || null,
    enode_device_id:
      event.device?.id ?? event.charger?.id ?? event.action?.targetId ?? null,
    payload: event,
  });
  ...
  if (
    event.event === "user:device:updated" ||
    event.event === "user:device:discovered"
  ) {
    ...
    try {
      await syncSingleDevice(companyId, deviceId, deviceTypeFromEvent(event));
    } catch {
      await syncCompanyDevices(companyId);
    }
    return;
  ...
  if (event.event === "charger:action:updated") {
