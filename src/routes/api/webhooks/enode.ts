...
/** POST /api/webhooks/enode — validated Enode webhook ingress. */
export const APIRoute = createAPIFileRoute("/api/webhooks/enode")({
  POST: async ({ request }) => {
    ensureEnodeEnv();
    ...
    try {
      ...
      return Response.json(
        { received: true, count: events.length, queued: true },
        { status: 202 },
      );
    } catch (err) {
      return Response.json(
