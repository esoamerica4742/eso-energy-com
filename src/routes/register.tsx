...
// Optional type interface for form data metrics
...
export default function AuthPortal() {
  ...
  const handleOnboardingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simulated secure enterprise authorization key dispatch delay
    setTimeout(() => {
      setIsLoading(false);
      setIsSubmitted(true);
    }, 1200);
        ...
        {!isSubmitted ? (
          <>
            ...
            <div className="text-center mb-10 relative z-10">
              ...
              <p class="text-zinc-500 text-xs tracking-wide max-w-sm mx-auto leading-relaxed">Enter credentials to establish your continuous commercial solar inverter tracking portfolio.</p>
            </div>

            {/* Interactive Functional Form */}
            <form className="space-y-6 relative z-10" onSubmit={handleOnboardingSubmit}>
              
              {/* Full Name Configuration Target */}
              <div>
              ...
              <div className="p-4 rounded-xl bg-zinc-900/20 border border-zinc-900/60 flex items-start gap-3">
                ...
                <p className="text-[11px] text-zinc-500 leading-normal tracking-wide">
                  <strong className="text-zinc-400 font-medium">Secured Encryption Onboarding:</strong> To preserve integrity, ESO uses cryptographic access tokens. No permanent password required. A dynamic verification link will be routed to your terminal instantly.
                </p>
              </div>
            ...
            <div className="mt-8 text-center text-xs text-zinc-600 relative z-10 tracking-wide">
              Already managing fleets? <a href="#terminal" className="text-amber-500/80 hover:text-amber-400 font-medium transition-colors ml-1">Access Terminal</a>
            </div>
          </>
        ) : (
          ...
          <div className="text-center py-10 animate-fade-in relative z-10">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 mb-6">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <h2 className="text-2xl font-light tracking-wide text-zinc-100 mb-3">Authorization Key Dispatched</h2>
            <p className="text-zinc-400 text-sm max-w-xs mx-auto leading-relaxed mb-1">
              A dynamic secure connection link has been sent to <span className="text-amber-500/90 font-medium">{formData.corporateEmail}</span>.
            </p>
            <p className="text-zinc-600 text-xs mt-4">Please verify your corporate terminal to continue your session.</p>
          </div>
        )}
      </div>
    </div>
