-- Enterprise telemetry logs (multi-tenant, deduped, realtime-friendly)
-- Supports strict 5-minute ingestion cadence with low-bandwidth delta fanout.

create extension if not exists pgcrypto;

-- Optional Timescale acceleration when extension exists.
do $$
begin
...
exception when others then
  -- keep migration portable on Supabase projects without Timescale enabled
  null;
end $$;
...
create or replace function public.debug_rls_tenant_context()
returns jsonb
language sql
stable
as $$
  select jsonb_build_object(
    ...
    'resolved_tenant', public.current_tenant_id()
  );
$$;

-- ---------------------------------------------------------------------------
-- Main telemetry table (partitioned by created_at month)
-- ---------------------------------------------------------------------------
...
declare
  ...
  m2 timestamptz := date_trunc('month', now()) + interval '2 month';
begin
  execute format(
    'create table if not exists public.telemetry_logs_%s partition of public.telemetry_logs for values from (%L) to (%L)',
    to_char(m0, 'YYYYMM'),
    m0,
    m1
  );
...
-- ---------------------------------------------------------------------------
-- Latest state cache and site summary cache (delta feeds)
-- ---------------------------------------------------------------------------
...
create index if not exists telemetry_logs_system_timestamp_idx
  on public.telemetry_logs(tenant_id, system_timestamp desc);
create index if not exists telemetry_logs_hash_idx
  on public.telemetry_logs(device_id, telemetry_hash, system_timestamp desc);
...
-- ---------------------------------------------------------------------------
-- Delta ingest function (change-only persistence)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Site summary refresh (for mobile low-bandwidth cards)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Offline detection for stale telemetry and alert insertion
-- ---------------------------------------------------------------------------
create or replace function public.telemetry_detect_offline(
...
begin
  update public.telemetry_latest_state
  set inverter_status = case
      when system_timestamp < now() - make_interval(mins => p_critical_minutes) then 'offline_critical'
      when system_timestamp < now() - make_interval(mins => p_stale_minutes) then 'offline_stale'
      else inverter_status
    end,
    updated_at = now()
  where system_timestamp < now() - make_interval(mins => p_stale_minutes);
  ...
  from public.telemetry_latest_state
  where system_timestamp < now() - make_interval(mins => p_critical_minutes);

  return jsonb_build_object('stale', v_stale, 'critical', v_critical);
end;
$$;

grant execute on function public.telemetry_detect_offline(integer, integer) to service_role;

-- ---------------------------------------------------------------------------
-- Retention / compression hooks
-- ---------------------------------------------------------------------------
create or replace function public.telemetry_retention_prune(p_days integer default 90)
returns integer
language plpgsql
security definer
set search_path = public
...
-- Optional Timescale compression policy (best effort)
do $$
begin
  if exists (select 1 from pg_extension where extname = 'timescaledb') then
    perform public.create_hypertable('public.telemetry_logs', 'created_at', if_not_exists => true, migrate_data => true);
    execute 'alter table public.telemetry_logs set (timescaledb.compress = true)';
  end if;
exception when others then
  null;
end $$;
...
drop policy if exists "Service insert telemetry_logs" on public.telemetry_logs;
create policy "Service insert telemetry_logs"
